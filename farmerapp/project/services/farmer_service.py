from repositories.farmer_repo import FarmerRepo
...
class FarmerService:
    ...
    def get_all_farmers(page: int, page_size: int):
        try:
            farmers, total_count = FarmerRepo.get_all_farmers(page, page_size)
    ...
    def get_farmer_by_id(farmer_id):
        try:
            farmer = FarmerRepo.get_farmer_by_id(farmer_id)
            if farmer:
    ...
    def get_farmers_by_crop(crop_grown):
        try:
            farmers = FarmerRepo.get_farmers_by_crop(crop_grown)
    ...
    def create_farmer(phone_number, name, language, country_id):
        try:
            farmer, error = FarmerRepo.create_farmer(phone_number, name, language, country_id)
            if error:
    ...
    def calculate_bill(fertilizer_name, quantity):
        try:
            ...
            schedule = ScheduleRepository.get_schedule_by_fertilizer(fertilizer_name)
    ...
    def update_farmer_by_id(id, phone_number, name, language, country_id):
        try:
            farmer, error = FarmerRepo.update_farmer_by_id(id, phone_number, name, language, country_id)
            if error:
                return {"error": error}, 400
    ...
    def delete_farmer_by_id(id):
        try:
            response, status = FarmerRepo.delete_farmer_by_id(id)
